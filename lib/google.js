...
const SHEET_HEADERS = [
  ...
  "Photo 2",
  "Photo 3",
  // Short header so the column stays narrow (~90px)
  "Invoice",
];

/** Short dropdown “button” labels (column stays narrow). */
const SEND_DEPOSIT_BUTTON_LABEL = "Send";
const SEND_DEPOSIT_SENT_LABEL = "✓ Sent";
const SEND_DEPOSIT_COLUMN_WIDTH_PX = 88;
...
async function ensureSendDepositButtonColumn(sheetId, tab, gid) {
  const actionCol = SHEET_HEADERS.indexOf("Invoice");
  if (actionCol < 0) return;

  const requests = [
    {
      // Narrow purple header
    ...
    // Clear checkbox validation leftovers
    {
      setDataValidation: {
        range: {
          sheetId: gid,
...
function buildOrderColumnValues(record) {
  ...
  return {
    ...
    // Empty until bakery chooses Send from the Invoice dropdown
    Invoice: "",
  };
}
...
async function appendOrderRow(record) {
  ...
  // Action cell empty — bakery opens dropdown and picks “Send” (fires onEdit reliably)
  const actionCol = SHEET_HEADERS.indexOf("Invoice");
  if (actionCol >= 0) {
    outRow[actionCol] = "";
  }
  ...
  // Narrow purple “Send” dropdown cell (not a checkbox, not a wide column)
  if (actionCol >= 0) {
    try {
      await sheetsApi("POST", `/v4/spreadsheets/${sheetId}:batchUpdate`, {
        requests: [
          ...
          },
          {
            repeatCell: {
              ...
              cell: {
                ...
                note: "Send Deposit Invoice — open dropdown and choose Send after reviewing Estimated Subtotal.",
              },
              fields:
      ...
      console.warn("[sheets] Invoice Send button on row 2 skipped:", e.message);
    }
  }
