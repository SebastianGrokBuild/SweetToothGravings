...
async function ensureHeaders() {
  ...
  // Keep Stripe Deposit as checkbox "button" column (even when headers already match).
  try {
    const gid = await getSheetGid(sheetId, tab);
...
/**
 * Write one order row mapped by header name so full details always land in the
 * correct columns (name, email, phone, date, line items, subtotal, deposit, notes).
 */
async function appendOrderRow(record) {
  ...
  // Truncate/pad to sheet width used by append
  ...
  // Log a compact summary for Render logs (helps debug column mapping)
  console.log(
    "[sheets] append order",
