...
async function appendOrderRow(record) {
  ...
  // Ensure Stripe Deposit cell is an unchecked checkbox (one-click Send Deposit Invoice)
  const stripeCol = SHEET_HEADERS.indexOf("Stripe Deposit");
  if (stripeCol >= 0) {
    try {
      await sheetsApi("POST", `/v4/spreadsheets/${sheetId}:batchUpdate`, {
      ...
      console.warn("[sheets] Stripe Deposit checkbox on row 2 skipped:", e.message);
    }
  }
