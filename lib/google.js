...
async function appendOrderRow(record) {
  ...
  const row = [
    ...
    "", // Stripe Deposit (Apps Script checkbox)
  ];
  ...
  await sheetsApi(
    "POST",
    `/v4/spreadsheets/${sheetId}/values/${encodeURIComponent(tab)}!A:${lastCol}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,
    { values: [row] },
  );
